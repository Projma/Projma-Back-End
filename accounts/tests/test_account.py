...
class TestProfile:
    ...
    def signup_and_signin_process(self, create_user, login_user, authenticate_user):
        create_user()
        response = login_user()
        assert response.status_code == status.HTTP_200_OK
    ...
    def test_authenticated_user_get_profile_returns_200(self, create_user, login_user, authenticate_user):
        api_client = self.signup_and_signin_process(create_user, login_user, authenticate_user)
        response = api_client.get('/accounts/profile/myprofile/')
        assert response.status_code == status.HTTP_200_OK
